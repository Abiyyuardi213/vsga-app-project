        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2 align-items-center">
                    <div class="col-sm-6">
                        <h1 class="m-0"><i class="fas fa-edit"></i> Ubah Makanan</h1>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="container-fluid">
                <div class="card shadow">
                    <div class="card-header bg-warning text-dark">
                        <h3 class="card-title"><i class="fas fa-pencil-alt"></i> Form Ubah Makanan</h3>
                    </div>
                    <div class="card-body">
                        @if(session('error'))
                        ...
                        <form action="{{ route('makanan.update', $makanan->id_makanan) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="stok" class="form-control @error('stok') is-invalid @enderror"
                                       value="{{ old('stok', $makanan->stok) }}" required>
                                @error('stok')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="id_restoran" class="form-control @error('id_restoran') is-invalid @enderror" required>
                                    ...
                                    @foreach($restoran as $r)
                                        <option value="{{ $r->id_restoran }}" {{ old('id_restoran', $makanan->id_restoran) == $r->id_restoran ? 'selected' : '' }}>
                                            {{ $r->nama_restoran }}
                                        </option>
                                    @endforeach
                                </select>
